#!/usr/bin/env python3
"""
Bidirectional Dry Mode Runner - "Alpha 2%" Strategy
Separate parallel paper-trading instance. Runs alongside Alpha 1% (dry_runner).
Direction signal: fractional differentiation (d=0.1, lookback 5) sign.
  fracdiff > 0  -> LONG
  fracdiff < 0  -> SHORT
TP/SL symmetric +/-2%, vertical horizon 15, 3% position sizing,
3-loss circuit breaker with 50-bar cooldown.

Usage:
    python3 bidir_runner.py              # Run continuously (60s interval)
    python3 bidir_runner.py --once       # Single cycle (testing)
    python3 bidir_runner.py --status     # Show current state
"""
...
def compute_fracdiff(prices):
    """Fractional differentiation (d=0.1) of a price series, return last value.
    Matches alpha_1percent._compute_fracdiff: gamma-based coefficients,
    truncated window of 200, centered by subtracting mean of first 20 bars.
    """
...
def direction_from_fracdiff(frac_val):
    if frac_val > 0:
        return 'long'
    elif frac_val < 0:
        return 'short'
...
def direction_from_sma_revert(prices, n=50):
    if len(prices) < n + 1:
        return None
    sma = np.mean(prices[-(n+1):-1])
    cur = prices[-1]
    if cur < sma:
        return 'long'
    elif cur > sma:
...
def direction_from_momentum(prices, k=10):
    if len(prices) < k + 1:
        return None
    ret = prices[-1] / prices[-1-k] - 1
    if ret > 0:
        return 'long'
    elif ret < 0:
        return 'short'
...
SIGNAL_TYPES = ('fracdiff', 'sma_revert', 'momentum')

def signal_direction(signal_type, history):
    if signal_type == 'fracdiff':
        frac = compute_fracdiff(history[-20:])
        return direction_from_fracdiff(frac)
    elif signal_type == 'sma_revert':
        return direction_from_sma_revert(history)
    elif signal_type == 'momentum':
        return direction_from_momentum(history)
    return None
...
def get_effective_equity(state, prices):
    ...
    for sym, pos in positions.items():
        if not isinstance(pos, dict) or sym not in prices:
            continue
        ...
        if entry is None or qty is None or qty <= 0:
            continue
        if direction == 'short':
            unrealized += (float(entry) - prices[sym]) * float(qty)
        else:
...
def run_cycle(state, signal_type='fracdiff'):
    now = datetime.utcnow()
    ...
    if state['consecutive_losses'] >= RC.max_consecutive_losses:
        ...
        try:
            notify_circuit_breaker(state, bot="alpha2")
        except Exception:
            pass
        return state
    prices = {}
    ...
    for s in list(state['open_positions'].keys()):
        if s in prices:
            ...
            if close_reason is not None:
                ...
                try:
                    notify_trade_close(trade, state, bot="alpha2")
                except Exception:
                    pass
    for s in ASSETS:
        if s not in state['open_positions'] and s in prices:
            ...
            if len(state['price_history'][s]) >= TB.vertical_horizon + 10:
                direction = signal_direction(signal_type, state['price_history'][s])
                if direction is None:
                    continue
                pos_val = state['capital'] * RC.max_position_pct
                qty = pos_val / prices[s]
                if direction == 'short':
                    tp_price = prices[s] * 0.98
                    sl_price = prices[s] * 1.02
                else:
                    tp_price = prices[s] * 1.02
                    sl_price = prices[s] * 0.98
                ...
                print(f"  [{now.strftime('%H:%M:%S')}] OPENED {s} {direction.upper()}: @ ${prices[s]:,.2f} | TP ${tp_price:,.2f} | SL ${sl_price:,.2f} | signal={signal_type}")
                try:
                    trade_info = {
                        'symbol': s, 'direction': direction,
                        'entry_price': prices[s], 'quantity': qty,
                    ...
                    notify_trade_open(trade_info, state, bot="alpha2")
                except Exception:
                    pass
...
def show_status():
    if not STATE_FILE.exists():
        print("  No bidir state found. Start with: python3 bidir_runner.py")
        return
    s = load_state()
    prices = {}
    for sym in ASSETS:
    ...
    print("="*70)
    print("  ALPHA 2% (BIDIRECTIONAL) DRY MODE STATUS")
    print("="*70)
        ...
        for sym, pos in positions.items():
            if isinstance(pos, dict):
                ...
                if direction == 'short':
                    ...
                    upnl = (ep - entry) * qty
                print(f"    {sym}: {direction.upper()} @ ${entry:,.2f} | Now ${ep:,.2f} | PnL ${upnl:+,.2f}")
    else:
        print("\n  No open positions")
    if s.get('trades'):
        print("\n  LAST 5 TRADES:")
        for t in s['trades'][-5:]:
            if isinstance(t, dict):
                print(f"    {t.get('symbol','?')}: {t.get('direction','?').upper()} {t.get('reason','?')} @ ${t.get('exit_price',0):,.2f} | PnL {t.get('pnl_pct',0):+.2%} (${t.get('pnl_dollars',0):+,.0f})")
    print("\n  LOGS:")
    print(f"    {TRADE_LOG}")
    print(f"    {EQUITY_LOG}")
    print("="*70)
...
def main():
    parser = argparse.ArgumentParser(description='Bidirectional Dry Mode Runner (Alpha 2%)')
    ...
    parser.add_argument('--signal', choices=SIGNAL_TYPES, default='momentum',
                        help='Direction signal: momentum (default), sma_revert, fracdiff')
    ...
    state = load_state()
    print("="*70)
    print("  ALPHA 2% BIDIRECTIONAL DRY MODE RUNNER")
    print("="*70)
    print(f"  Assets:   BTC + ETH")
    print(f"  Signal:   {args.signal} (fracdiff d={FRAC_DIFF_ORDER} / sma_revert / momentum)")
    ...
    while running:
        cycle += 1
        try:
            state = run_cycle(state, signal_type=args.signal)
            save_state(state)
            if args.once:
